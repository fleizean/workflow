 ...
 * named a stale elapsed time and a persistFailing that flipped while the user was elsewhere never arrived - the
 * confirm promised "Quitting keeps it" while the disk was refusing writes.
 *
 * A plain function taking its sink, rather than a hook: that is what lets tests/shell-tick-lifetime.test.ts mount
 * this very subscription in node and read the confirm back.
 */
