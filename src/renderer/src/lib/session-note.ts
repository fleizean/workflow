 ...
 * is one nobody has been asked about yet; an empty string is one that was asked and had nothing to say. That is
 * what lets the prompt stop.
 *
 ...
 * repository. WR-02 is the other end of the same rule: the save form wrote NULL for an empty note too, so the app
 * itself could create a row indistinguishable from a pending one.
 *
 ...
 *  - except on an edit of a row whose note is already NULL: that row is still one nobody has answered, and saving
 *    a change to its duration is not an answer. A field nobody typed into is never written - the same rule
 *    settings-view.ts's reviewDraft follows, and the same one lib/duration.ts follows for the duration itself.
 */
