 ...
 * opens on top, and Escape to clear it also discards the session name, note and date just typed.
 *
 * A module rather than a counter inside the component, so "the topmost one answers" is a rule that can be run
 * (tests/modal-stack.test.ts) instead of read.
 */
